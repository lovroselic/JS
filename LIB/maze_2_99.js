...
/*
TODO:
  GridArray (old Gridmap) pushed to external - pacDungeon will not work
  pathNode and nodeMap pushed to external
...
class MasterDungeon {
  /*
  grid ~ binary string representation
  featureGridArray ~ GridArray - static features
  */
  ...
  carveMaze(start, protectSeed = false) {
  ...
  /*toGridArray(size = 1){
    if (this.featureGridArray === null){
      this.featureGridArray = new GridArray(this.width, this.height, size);
    }
    for (let y = 0; y < this.height; y++){
      for (let x = 0; x < this.width; x++){
        let grid = new Grid(x, y);
        if (this.isMazeWall(grid)){
          this.featureGridArray.toWall(grid);
        }
      }
    }
...
class PacDungeon extends MasterDungeon {
  constructor(sizeX, sizeY) {
    ...
    */
    ...
    this.toGridArray();
    this.featureGridArray.reserve(this.bonus);
    this.featureGridArray.reserve(this.startPoint);
    this.featureGridArray.reserve(this.rooms[0].door[0]);
  }
  symCopy() {
    for (let y = this.minY; y <= this.maxY; y++) {
...
var MAZE = {
  ...
  create: function (sizeX, sizeY, start) {
    return MAZE.configure(new Maze(sizeX, sizeY, start));
  }
};
var PACDUNGEON = {
